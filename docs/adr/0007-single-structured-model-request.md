...
inputs: [docs/prd.md, docs/intents/0005-check-my-summary.md, docs/intents/0006-ask-my-course.md, docs/intents/0007-quiz-me.md, docs/intents/0008-see-my-readiness.md, docs/adr/0001-structural-retrieval.md, docs/adr/0002-citation-validated-against-scope.md, CLAUDE.md]
updated: 2026-09-08
---
...
All four model-using requirements — 0005, 0006, 0007, 0008 — act on material the learner has
already chosen, per ADR 0001, so none of them involves open-ended exploration. A third
constraint bears on 0005 specifically: the learner's own account of a session must never be
written for them.
...
for requirement 0005 it returns an assessment of what the learner's summary missed or has
...
particularly on requirement 0005, where the specificity bar is high. A single pass also leaves
...
makes requirement 0009's cost-per-learner question answerable at all. Latency is one round
trip. And the response shape for 0005 can be constrained so that producing the learner's
summary is not merely discouraged but structurally unavailable.

**We will know it was wrong if:** a single pass cannot meet requirement 0005's bar — feedback
naming the specific omission and where it lives — or 0007's bar, eight in ten questions
...
|---|---|
| 0005 | One request; the response carries an assessment, never a model summary the learner could adopt as theirs |
| 0006 | One request; the response carries the structured citations ADR 0002 validates |
| 0007 | One request per practice set, over an enumerated session scope |
| 0008 | One request; evidence is summarised, never predicted as a grade |
| 0009 | Cost must be attributable per action so cost per learner can be reported |
