...
inputs: [docs/prd.md, docs/intents/0005-capture-a-session.md, docs/intents/0007-ask-my-course.md, docs/intents/0010-know-its-working.md, CLAUDE.md]
updated: 2026-09-08
---
...
Three PRD constraints meet on requirement 0005 and pull the same way. Capture must complete on
a phone, outdoors, on a poor connection. Every act of capture must return something visible
within seconds. Attached material has hard ceilings on size and page count. Requirement 0005's
...
failures are recorded against the material, so requirement 0010 can surface them to the
operator and requirement 0005 can tell the learner what happened and what to do.

## Rejected options

### Read the material before acknowledging the capture

A large document cannot be read inside the two minutes requirement 0005 demands, on the
...
answerable. Requirement 0005's second open question asks what the learner sees during that
window and what they have been promised once they close the page; this ADR deliberately does
not answer it, because it is a spec question for 0005. Requirement 0007 must also decide
...
|---|---|
| 0005 | Capture returns before material is read; the learner must be told something useful in the gap, and told later if it failed |
| 0007 | Scope must account for material not yet readable — excluded, or reported, but never silently treated as read |
| 0009 | Evidence counts must not credit material that failed to process |
| 0010 | Processing failures are operational events, visible to the operator inside 24 hours without being reported |
